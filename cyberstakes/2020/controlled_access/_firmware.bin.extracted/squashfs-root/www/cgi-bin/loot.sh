...
<body>
  <div class="topnav" id="myTopnav">
    ...
    <a href="/cgi-bin/loot.sh" class="active">Loot</a>
  ...
  <div class="container">
    <div class="row">
      <div class="eleven column" style="margin-top: 5%">
        <h4>Loot</h4>
        <p>
          <pre>$(find /root/loot -type f -exec echo {} \; | while read line; do echo "<a href=\"$line\">$line</a>"; done)</pre>
        </p>
        <p> 
          <form action="/cgi-bin/loot.sh?DEL" method="POST" enctype="application/x-www-form-urlencoded">
          <input type="hidden" id="DEL" "name="DEL" value="DEL">
          <input type="submit" class="button-primary" value="DELETE ALL LOOT" onclick="return confirm_delete()">
          </form>
        </p>
      </div>
    </div>
